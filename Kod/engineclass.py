...
class manipulator:
    ...
    def main_window(self, main):
        '''
        Metoda wykonywana po stworzeniu glówmnego okna przekazujaca je w celu komunikacji
        :param main: wskaznik do glównego okna
        '''
        self.main = main

    def center(self):
        '''
        Metoda właczajaca referecing mode i nastempnie centrujaca manipulator
        '''
        self.ustaw_referencing_mode()
        self.pobierz_pozycje_osi()
        self.reference_axes()
        self.poczekaj_na_osiogniecie_celu()

    def _pobierz_skonwertowane_pozycje(self, osie='XYZ'):
        '''
        metoda konwertuje pozycje zadanych osi na tablice charów zrozumiała przez manipulator
        :param osie: string zawierajacy nazwy osi
        :return: wskaznik do tablicy charów
        '''
    ...
    def ustaw_referencing_mode(self, osie='xyz', tryb=True):
        '''
        metoda włącza lub wyłącza mod referecyjny dla osi manipulatora
        :param osie: osie dla których włączamy tryb referecji
        :param tryb: wartosc bitowa właczajaca lub wyłaczajaca mod
        ...
        for os_c in osie:
            ...
            bool_array = self._stwurz_tablice_booli(size=1, values=tryb)
    ...
    def odczyt_pozycji():
        ...
        :return: słownik zawierajacy pozycje
        '''
    ...
    def split_axes_positions(position_dict):
        '''
        Statyczna metoda konwertujaca odczytany slownik na dwie niezalezne tablice
        :param position_dict: słownik z osiami
    ...
    def ustaw_abs_positions_z_file(self):
        ...
        natempnie wyłacza tryb referecyjny i
        ustawia ze manipulator znajduje sie na zadanych kordynatach
        :return: status czy udało sie ustawic odczytane pozycje czy nie
        '''
        ...
        return self.ustaw_absolutne_pozycje(axes=axes, positions=positions)
    ...
    def ustaw_absolutne_pozycje(self, axes='xyz', positions = None):
        '''
        Metoda wysyła do manipulatora zadane pozycje w celu przesuniecia manipulatora na nie
        :param axes:
        :param positions:
        :return: status czy sie udało wykonac przemiescenie
        '''

        #sprawdzenie czy zadano jakiekolwiek osie do przemiecenia
        if positions == None:
            return None
        ...
        # wykonanie przemiescenia
        c_id = self._convert_id(self.controller_id)
        sz_axes = self._pobierz_skonwertowane_pozycje(axes)
        c_double_array = self._stwurz_tablice_doubli(len(axes), positions)
    ...
    def zaladuj_sterowniki(filename='C848_DLL.dll'):
        '''
        Statyczna metoda odczytujaca i ładujaca dynamiczna biblioteke do komunikacji z silnikiem
    ...
    def podloczenie_controller(self):
        ...
        #conect_fun port_szeregowy predkosc bitów
    ...
    def przerwij_poloczenie(self):
        '''
        Przeywa poloczenie z kontrolerem silników
        :return:
        '''
    ...
    def _conwert_axes(axes='XYZ'):
        '''
        konwertuje "xyz" string do "ABC" string zrozumiałego dla kontrolera
        #A = x
        #B = y
        #C = z
        '''
    ...
    def _pobierz_skonwertowane_axes(self, axes='XYZ'):
        ...
        return wskaznik na tablice charów zrozumiała przez kontroler
        '''
    ...
    @staticmethod
    def _stwurz_tablice_booli(size=1, values=0):
        '''
        tworzyu tablice booli o zadanej wielkosci i zwraca cpointer do niej
        '''
        b = (ctypes.c_bool * size)(*[values] * size)
        return ctypes.cast(b, ctypes.POINTER(ctypes.c_bool))

    @staticmethod
    def _stwurz_tablice_doubli(size=1, positions=None):
        '''
        tworzyu tablice doubli o zadanej wielkosci i zwraca cpointer do niej
        '''
        if positions == None:
            positions = [25.0] * size
    ...
    def przsun_manipulator_do_zadanej_pozycji(self, axes='xyz', positions=[25.0, 25.0, 25.0]):
        '''
        Metoda zadajaca przesuniecie zadanych osi na zadana pozycje
        :param axes: chare osi
        :param positions: pozycje dla odpowiednich osi
        :return: status powodzenia
        '''
        return self._przsun_manipulator(axes, positions)

    def _przsun_manipulator(self, axes, positions):
        '''
        prywatnan metoda zadajaca przesuniecie zadanych osi na zadana pozycje
        :param axes: chare osi
        :param positions: pozycje dla odpowiednich osi
        :return: status powodzenia
        '''
    ...
    def pobierz_pozycje_osi(self, axes='xyz'):
        ...
        c_id = self._convert_id(self.controller_id)
        sz_axes = self._pobierz_skonwertowane_axes(axes)
        c_double_array = self._stwurz_tablice_doubli(size=len(axes))
    ...
    def wytpisz_aktualna_pozycje_manipulatora(self):
        '''
        metoda czeka az manipulator przestanie sie poruszac po osiongnieciu pozycji wypisze pozycje
        :return:
        '''
    ...
    def sprawdz_czy_u_celu(self, axes='xyz'):
        '''
        metoda sprawdza czy manipulator osiognoł zadana pozycje
        '''
        ...
        for c in axes:
            ...
            bool_array = self._stwurz_tablice_booli(size=1)
    ...
    def poczekaj_na_osiogniecie_celu(self):
        '''
  ...
  ########################metody odbierajace wysłąne poleca ruchu################
    ...
    def przesun_w_gore(self, krok):
        '''
        metoda przsuwajaca pozycje manipulatora o zadany krok i oczekujaca az zostanei osiogniety cel
        :param krok:
    ...
    def move_axes_to_abs_woe_ofset(self,axes,tab):
        ...
        :param axes: osie które przesuwamy
        :param tab: tablica wartosci o które przesuwamy manipulator
        '''
